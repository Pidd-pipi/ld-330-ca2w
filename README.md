...
- 跨机构调阅闭环：患者可为指定医疗机构授予限时查阅权；医生调阅前系统核验授权有效且覆盖本次用途，并记录调阅对象与时间；撤回、到期或超范围一律拒绝；同一授权重复调阅只追加审计、不生成重复授权；撤回与再次授权并发时通过乐观锁保证只有一处成功；档案页展示有效授权、调阅记录与失败提示，数据落库刷新后可回查。
